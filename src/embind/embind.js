...
var LibraryEmbind = {
  ...
  $ensureOverloadSignatureTable: (proto, methodName, humanName, numArguments) => {
    ...
    if (undefined !== proto[methodName].overloadTable && undefined !== proto[methodName].overloadTable[numArguments] && undefined === proto[methodName].overloadTable[numArguments].signatures) {
      ...
      proto[methodName].overloadTable[numArguments] = function() {
        var args = Array.prototype.slice.call(arguments);
        var keys = proto[methodName].overloadTable[arguments.length].signaturesArray;
        var signature = getSignature(args, keys);
        // TODO This check can be removed in -O3 level "unsafe" optimizations.
        if (!proto[methodName].overloadTable[arguments.length].signatures.hasOwnProperty(signature)) {
            var signatures = proto[methodName].overloadTable[arguments.length].signaturesArray.map(sig => '(' + sig.map(s => typeof s === 'string' ? s : registeredTypes[s].name) + ')');
        ...
        return proto[methodName].overloadTable[arguments.length].signatures[signature].apply(this, arguments);
      };
...
//    }
  ...
  _embind_register_function__deps: [
    ...
    '$throwUnboundTypeError', '$whenDependentTypesAreResolved', '$getFunctionName', '$cppTypeToJsType'],
    ...
    whenDependentTypesAreResolved(
      ...
      function(base) {
        ...
        var constructor = createNamedFunction(name, function() {
          ...
          var body = undefined;
          if (undefined !== registeredClass.constructor_body[arguments.length]) {
            if (undefined !== registeredClass.constructor_body[arguments.length].func) {
              body = registeredClass.constructor_body[arguments.length].func;
            } else {
              var args = Array.prototype.slice.call(arguments);
              var keys = registeredClass.constructor_body[arguments.length].signaturesArray;
              var signature = getSignature(args, keys);

              body = registeredClass.constructor_body[arguments.length].signatures[signature];
            }
          }
          ...
          if (undefined === body) {
            if (undefined === registeredClass.constructor_body[arguments.length]) {
              ...
              var signatures = registeredClass.constructor_body[arguments.length].signaturesArray.map(sig => '(' + sig.map(s => typeof s === 'string' ? s : registeredTypes[s].name) + ')');
              var params = args.map(arg => (typeof arg === 'object' && arg.constructor && arg.constructor.name) ? arg.constructor.name : typeof arg);
